...
const Nav = () => {
  ...
  return (
   ...
   <img className={`${menuCollapse ? "block":"hidden"} w-64 h-14 md:block lg:h-[4.2rem] lg:w-[32rem] lg:relative lg:right-14`}  src={GoodRenewable} alt="goodRenewable logo"/>
    { !menuCollapse &&   <BookCallBtn text="Book a Call" className="py-2 book-call-btn border-2   hover:bg-black hover:text-white  md:hidden" fill="#152223" /> }  
  { !menuCollapse &&  <img onClick={hanldeMenuHamburgerClicked} className="md:hidden" src={menuBurger} alt="menu burger icon" />}    
     { menuCollapse && <img onClick={hanldeCloseIconClicked} className='md:hidden px-3' src={closeIcon} alt="close icon" />}  
        ...
        <li className='relative top-10 md:top-0   '><a  href='#'><BookCallBtn className='book-call-btn border-2 lg:px-10  hover:bg-black hover:text-white lg:ml-20' text="Book a Call" fill="#152223" /></a> </li>
     </ul> 
      </nav>
    </header>
  )
